...
typedef struct xmem_t{
    uint8_t xor_val8                                   _ALIGN; //scalar offs:0 size:1
    uint16_t xor_val16                                 _ALIGN; //scalar offs:4 size:2
    ...
    uint8_t no_p_arr[2]                                _ALIGN; //scalar offs:64 size:2
    uint8_t no_q_arr[2]                                _ALIGN; //scalar offs:68 size:2

uint8_t xxxxx_paddingA[8122];

    char filter_map[MAX_FILTER_SIZE * MAX_FILTER_SIZE] _ALIGN; //array offs:8192 size:64
    char pixel[(MAX_WIDTH_SIZE + MAX_FILTER_SIZE -1 ) * (MAX_HEIGHT_SIZE + MAX_FILTER_SIZE - 1)] _ALIGN; //array offs:8256 size:5041
    int sum [MAX_WIDTH_SIZE * MAX_HEIGHT_SIZE]         _ALIGN; //array offs:13300 size:16384
    int arr_s1[10]                                     _ALIGN; //array offs:29684 size:40
    int arr_s2[10]                                     _ALIGN; //array offs:29724 size:40
    int arr_d1[10]                                     _ALIGN; //array offs:29764 size:40
    int big_array[10000]                               _ALIGN; //array offs:29804 size:40000
    uint8_t pix_base[1920*1080]                        _ALIGN; //array offs:69804 size:2073600

// Total scalar size: 63
// Total array size: 2135209
// Total cyclic size: 0
